...
const ActiveTenderForm = ({ onAddTender }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Yeni aktif ihale verisini oluştur
    const newTender = {
      id: Date.now(),
      name,
      difficulty,
      ...
      deadline,
      candidates: candidates.split(',').map(candidate => candidate.trim()), // Adaylar virgülle ayrılmış liste
    };
    onAddTender(newTender); // Yeni ihaleyi üst bileşene gönderiyoruz
    // Formu temizle
    setName('');
    setDifficulty('');
    setFunding('');
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-800 p-6 rounded-lg shadow-lg">
      <h2 className="text-xl font-bold text-gray-100 mb-4">Yeni Aktif İhale Ekle</h2>
      <div className="mb-4">
...
export default ActiveTenderForm;
